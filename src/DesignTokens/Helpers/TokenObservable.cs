...
internal sealed class TokenObservable<TValue, TKey>(
...
) : IObservable<TValue>
{
    ...
    {
        private readonly TokenObservable<TValue, TKey> _owner;
        private readonly IObserver<TValue> _observer;
        private readonly TokenHostState<TValue, TKey> _hostState;
        private bool _isDisposed;

        public Subscription(TokenObservable<TValue, TKey> owner, IObserver<TValue> observer)
        {
            _owner = owner;
            _observer = observer;
            _hostState = new TokenHostState<TValue, TKey>(_owner._context, _owner._application);
